...
static double energy_cb_from_em(
    ggml_stage st,
    perf_group pg,
    int cpu_khz,
    int mem_khz,
    ...
    const EnergyModel * em = reinterpret_cast<const EnergyModel *>(user);

    // predict_energy_j는 latency_us 단위로 받는다고 네 selector.cpp에 이미 맞춰놨었지
    const double latency_us = lat_ms * 1000.0;

    return predict_energy_j(*em, st, pg, cpu_khz, mem_khz, latency_us);
}

energy_predict_cb_t get_energy_cb() {
    return &energy_cb_from_em;
}
